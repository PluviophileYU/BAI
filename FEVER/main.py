    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            if args.task_name == 'fever':
                # ood_set1 = 'symv1test'
                ood_set2 = 'symv2test'
            # ood_result_1 = evaluate(args, model, tokenizer, set=ood_set1)
            ood_result_2 = evaluate(args, model, tokenizer, set=ood_set2)
            logger.info("-------------{} Results-------------".format(args.task_name.upper()))
            logger.info("IID performance: {}".format(iid_result))
            logger.info("OOD performance: {}".format(ood_result_2))
