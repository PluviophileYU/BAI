            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                # if global_step % args.save_steps == 0:
                    ...
                    if now_acc > best_acc:
                        best_acc = now_acc
                        logger.info(
                            "Now Best checkpoint and its dev acc result: {}, {}".format(global_step, round(best_acc*100,2)))
...
def evaluate(args, model, tokenizer, set='dev'):
    ...
    # HANS only has two classes
    if set == 'hans':
        # preds = np.concatenate(((preds[:, 0] + preds[:, 2]).reshape(-1, 1), preds[:, 1].reshape(-1, 1)), axis=1)

        # take max of non-entailment rather than taking their sum
        preds[:, 0] = preds[:, [0, 2]].max(axis=1)
        preds = preds[:, :2]
