...
def train(args, model, train_dataset, tokenizer, tb_writer, exec_time):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                # if global_step % args.save_steps == 0:
                    now_ood = evaluate(args, model, tokenizer, ood_set)
                    now_iid = evaluate(args, model, tokenizer, iid_set)
                    logger.info(
                        'iid performance: {} / {} / {}'.format(now_iid['dupl'], now_iid['non-dupl'], now_iid['accuracy?']))
                    logger.info(
                        'ood performance: {} / {} / {}'.format(now_ood['dupl'], now_ood['non-dupl'], now_ood['accuracy?']))
                    if args.iid_valid:
                        now_acc = now_iid['accuracy?']
                    else:
                        now_acc = now_ood['accuracy?']
                    if now_acc > best_acc:
                        best_acc = now_acc
                        logger.info(
                            "Now Best checkpoint and its weighed ood result: {}, {}".format(global_step, now_acc))
    ...
    now_ood = evaluate(args, model, tokenizer, ood_set)
    now_iid = evaluate(args, model, tokenizer, iid_set)
    logger.info('iid performance: {} / {} / {}'.format(now_iid['dupl'], now_iid['non-dupl'], now_iid['accuracy?']))
    logger.info('ood performance: {} / {} / {}'.format(now_ood['dupl'], now_ood['non-dupl'], now_ood['accuracy?']))
    if args.iid_valid:
        now_acc = now_iid['accuracy?']
    else:
        now_acc = now_ood['accuracy?']
...
def evaluate(args, model, tokenizer, set='dev'):
    ...
    result = classification_report(out_label_ids, preds, target_names=['non-dupl', 'dupl'], digits=4, output_dict=True)

    acc = {}
    acc['dupl'] = round(result['dupl']['precision'] * 100, 2)
    acc['non-dupl'] = round(result['non-dupl']['precision'] * 100, 2)
    acc['weighted'] = round(result['weighted avg']['precision'] * 100, 2)
    acc['accuracy?'] = round(100 * (preds == out_label_ids).mean(), 2)
    return acc
