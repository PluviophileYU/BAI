    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        if args.task_name == 'qqp':
            ...
            if evaluate == 'train':
                ...
                examples = Processor.get_paws_examples(args.data_dir)
...
class QqpProcessor(DataProcessor):
    ...
    def get_paws_examples(self, data_dir):
        """See base class."""
        return self._create_paws_examples(
            self._read_tsv(os.path.join(data_dir, 'qqp', "paws_qqp/processed_dev_and_test_.tsv")), "paws")
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            try:
                ...
                continue
            examples.append(
                InputExample(guid=i-1, text_a=text_a, text_b=text_b, label=label))
        return examples

    def _create_paws_examples(self, lines, set_type):
        examples = []
        for (i, line) in enumerate(lines):
            if i == 0:
                continue
            ...
            text_b = line[2]
            label = line[3]
            text_a = text_a.strip('b').strip('\\\'')
            text_b = text_b.strip('b').strip('\\\'')
            examples.append(
